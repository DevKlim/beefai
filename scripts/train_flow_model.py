...
DEFAULT_SAVE_INTERVAL_STEPS = 1000 
DEFAULT_SAVE_CHECKPOINT_EVERY_N_EPOCHS = 20 
...
def train_full_model(): 
    ...
    save_checkpoint_every_n_epochs = model_params_yaml.get("save_checkpoint_every_n_epochs", DEFAULT_SAVE_CHECKPOINT_EVERY_N_EPOCHS)
    ...
    log_runs_dir = os.path.join(checkpoint_dir, 'runs_full') 
    ...
    gpt_config = FlowGPTConfig(
        ...
        block_size=model_params_yaml["block_size"], 
        n_layer=model_params_yaml["n_layer"],       
        n_head=model_params_yaml["n_head"],         
        n_embd=model_params_yaml["n_embd"],         
        max_segment_types=model_params_yaml["max_segment_types"], 
        max_intra_line_positions=model_params_yaml["max_intra_line_positions"], 
        dropout=model_params_yaml.get("dropout", 0.1), 
        bias=model_params_yaml.get("bias", True),      
    ...
    train_data_path = data_params_yaml.get("train_data_path")
    if not train_data_path:
        ...
        if not tokenized_data_output_dir:
            print(f"ERROR: 'tokenized_data_output_dir' or 'train_data_path' not specified in {DATA_CONFIG_FULL_PATH}"); writer.close(); return
        train_data_path = os.path.join(tokenized_data_output_dir, train_data_filename)

    val_data_path = data_params_yaml.get("val_data_path")
    if not val_data_path:
        tokenized_data_output_dir = data_params_yaml.get("tokenized_data_output_dir")
        val_data_filename = data_params_yaml.get("val_data_filename", "val_full.pt")   
        if not tokenized_data_output_dir:
            print(f"Warning: 'tokenized_data_output_dir' or 'val_data_path' not specified in {DATA_CONFIG_FULL_PATH}. Validation data might not be found if '{val_data_filename}' is not in project root.")
            val_data_path = os.path.join(tokenized_data_output_dir if tokenized_data_output_dir else ".", val_data_filename)
        else:
    ...
    val_loader = None
    if os.path.exists(val_data_path) and len(train_dataset) > 0:
        ...
        if len(val_dataset) > 0:
            ...
            print(f"Warning: FULL validation data file '{val_data_path}' is empty. Proceeding without validation.")
        ...
        print(f"Warning: Training dataset is empty. Skipping validation loader creation.")

    optimizer = optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=weight_decay, betas=(0.9, 0.95)) 
    
    if len(train_loader) > 0 and epochs > 0 :
        num_optimizer_steps_per_epoch = (len(train_loader) + grad_accumulation_steps - 1) // grad_accumulation_steps
        total_steps_for_scheduler = num_optimizer_steps_per_epoch * epochs
    else: 
        total_steps_for_scheduler = 0
        
    scheduler = OneCycleLR(optimizer, max_lr=learning_rate,
                           total_steps=max(1, total_steps_for_scheduler), 
                           pct_start=0.1, 
                           anneal_strategy='cos') 
    ...
    if len(train_loader) == 0:
        print("ERROR: Training data loader is empty. Cannot start training.")
        writer.close(); return
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(progress_bar):
            ...
            if (i + 1) % grad_accumulation_steps == 0 or (i + 1) == len(train_loader):
                ...
                if total_steps_for_scheduler > 0:
                     scheduler.step() 
        ...
        if (save_checkpoint_every_n_epochs > 0 and (epoch + 1) % save_checkpoint_every_n_epochs == 0) or \
           (epoch + 1) == epochs:
