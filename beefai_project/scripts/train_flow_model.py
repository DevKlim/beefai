...
DEFAULT_SAVE_CHECKPOINT_EVERY_N_EPOCHS = 20 # Default if not in YAML
...
def train_full_model(): # Renamed function for clarity
    ...
    train_data_filename = data_params_yaml.get("train_data_filename", "train_full.pt") # Default filename
    val_data_filename = data_params_yaml.get("val_data_filename", "val_full.pt")     # Default filename

    # Use train_data_path and val_data_path directly from data_config if they exist and are full paths
    # Otherwise, construct them from tokenized_data_output_dir and filenames
    ...
    if not val_data_path:
        if not tokenized_data_output_dir:
            # This case is less critical if val_data_path is optional, but good to be complete
    ...
    for epoch in range(epochs):
        ...
        if val_loader: # Perform validation at the end of each epoch as well
            evaluate(model, val_loader, DEVICE, pad_token_id, USE_AMP, "FULL", writer, global_step) 
            model.train()
        
        # Conditional epoch checkpoint saving
        if (epoch + 1) % save_checkpoint_every_n_epochs == 0 or (epoch + 1) == epochs:
    ...
    # Save final model (this might be redundant if the last epoch triggered the conditional save, but ensures a 'final' model exists)
